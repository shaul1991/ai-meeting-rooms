...
@section('title', '회의실 수정')

@section('content')
<div class="max-w-2xl mx-auto">
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-2xl font-bold text-gray-900">회의실 수정</h1>
    ...
    <form action="{{ route('admin.rooms.update', $room->id()->value()) }}" method="POST" class="bg-white shadow-md rounded-lg p-6 space-y-6">
        @csrf
        @method('PUT')
        ...
        <div>
            ...
            <textarea id="description"
                      name="description"
                      rows="3"
                      class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 @error('description') border-red-500 @enderror">{{ old('description', $room->description()) }}</textarea>
            @error('description')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="grid grid-cols-2 gap-4">
            ...
            <div>
                ...
                <input type="number"
                       id="price_per_slot"
                       name="price_per_slot"
                       value="{{ old('price_per_slot', $room->pricePerSlot()->amount()) }}"
                       min="0"
                       required
        ...
        <div>
            ...
            <select id="group_id"
                ...
                @foreach($groups as $group)
                    <option value="{{ $group->id()->value() }}"
                            {{ old('group_id', $room->groupId()?->value()) == $group->id()->value() ? 'selected' : '' }}>
                        {{ $group->name() }}
                    </option>
                @endforeach
            </select>
        ...
        <div>
            ...
            @php
                ...
                $operatingHours = $room->operatingHours()->toArray();
            @endphp
            <div class="space-y-3">
                @foreach($days as $dayNum => $dayName)
                    @php
                        $dayHours = $operatingHours[$dayNum] ?? null;
                    ...
                    <div class="flex items-center space-x-4">
                        ...
                        <input type="time"
                               ...
                               value="{{ old("operating_hours.$dayNum.start", $start) }}"
                        ...
                        <input type="time"
                               ...
                               value="{{ old("operating_hours.$dayNum.end", $end) }}"
                               class="px-3 py-1 border border-gray-300 rounded focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500">
                        <label class="flex items-center">
                            <input type="checkbox"
                                   name="operating_hours[{{ $dayNum }}][is_closed]"
                                   value="1"
                                   {{ old("operating_hours.$dayNum.is_closed", $isClosed) ? 'checked' : '' }}
